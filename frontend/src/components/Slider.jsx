            ...
            <div className='h-full flex overflow-x-hidden overflow-y-hidden '>
                {
                    sliderData.map(item => (
                        <div className={`flex flex-shrink-0 items-center h-full w-full  ${item.bgColor}`} style={{ transform: `translateX(-${cursor * 100}%)`, transition: 'transform 1.5s ease' }} key={item.key}>
                            <div className='flex-1 h-full w-full p-5 md:block hidden '>
                                <img src={item.img} alt="" className='h-[70%] mt-10 mx-auto' />
                            </div>
                            <div className=' md:flex-1 p-[50px]'>
                                <h1 className='font-bold md:text-left text-[50px] text-center '>{item.title}</h1>
                                <p className='md:text-left text-[30px] tracking-wider mt-[50px] text-center'>{item.desc}</p>
                                <div className='flex justify-center md:justify-start'>
                                    <button className='border-2 border-gray-400 bg-transparent p-3  font-medium mt-[50px] shadow-md'>SHOW MORE
                                    </button>
                                </div>
